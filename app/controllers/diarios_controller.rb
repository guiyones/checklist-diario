class DiariosController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @diario.update(diario_params)
        format.html { redirect_to @diario, notice: "Diario was successfully updated.", status: :see_other }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to diarios_path, notice: "Diario was successfully destroyed.", status: :see_other }
  ...
  private
    ...
    def set_diario
      @diario = Diario.find(params.expect(:id))
    end
